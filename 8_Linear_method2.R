...
func_proba_Y_2<- function( Y, X12,X34,p1,p2,
  ...
  eta_mat= expo_beta  (X12,X34, p1,p2 ,beta0)  
  
  Y_mat <- matrix(Y, nrow = nA, ncol = nB, byrow = TRUE)
  
  m <-  (Y - mu2X) # nB
  
  m_mat =  matrix(m, nrow = nA, ncol = nB, byrow = TRUE) # nA x nB
  
  num <-  (Y_mat - eta_mat)   #nA X nB
  
  density_num=matrix(0,nA,nB)
  density_m_mat=matrix(0,nA,nB)
  #
  for (j in 1:nB) {
    density_num[,j] = dnorm(num[,j],0, sqrt(variance1))
    density_m_mat[,j]= dnorm(m_mat[,j],0, sqrt(variance0))
  }
  
  denom= (matrix_id* density_num ) +  ( (1-matrix_id)*density_m_mat)
  
  denom[denom == 0] <- .Machine$double.eps
  
  matrix_proba_Y_2= (matrix_id* density_num )/ denom
...
linear_method_2 <- function( p1,p2, Y, matrix_proba_Y_2,beta0, mu2_0,
                             X12, X34){
  ...
  ## v_j and V
  vj <- colSums( 1-matrix_proba_Y_2) #nB
  V <- diag(vj)
  
  ## mu2
  mu2_new <-as.vector( solve(
    t(X34) %*% V %*% X34,
    t(X34) %*% V %*% Y)
  )
  
  ## sigma0
  sigma_0 <- as.numeric( sum(vj * (Y - X34 %*% mu2_0)^2) / sum(1-matrix_proba_Y_2) )
  
  #   beta
  beta_new=as.vector( solve( t(X_M) %*% (vect_pi * X_M), 
                         t(X_M) %*% (vect_pi * Y_M)) )
  ...
  return(list(beta =beta_new, mu2 = mu2_new, sigma_1=sigma_1, 
              sigma_0=sigma_0))
}
 ...
 # loglik <- numeric(maxits)
  ...
  while ((!converge) && (it < maxits)){ 
   ...
   # loglik[it + 1] <- sum(log(denom_safe))
    ...
    #maximization
    estime = linear_method_2 ( p1,p2, Y,matrix_proba_Y_2, beta0_old,mu2_old,
                               X12,X34)
    beta0 = estime$beta
